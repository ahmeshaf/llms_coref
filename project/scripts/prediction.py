...
from coval.coval.eval.evaluator import (b_cubed, ceafe, evaluate_documents,
                                        lea, muc)
...
def run_llm_lh(dataset: str, split: str, 
               prompt: PromptTemplate,
               parser: StructuredOutputParser,  
               lh_threshold: float = 0.5) -> Tuple[List, Dict]:
    """
    ...
    (tps, fps, tns, fns), (tps_trans, fps_trans, tns_trans, fns_trans)= results[split_index_map[split]]
    prediction_pairs = tps + fps

    prediction_list, prediction_dict = llm_coref(prediction_pairs, evt_mention_map, \
                            prompt=baseline_prompt, parser=baseline_output_parser,)
...
def llm_coref(prediction_pairs: List[Tuple[str, str]],
              ...
              save_folder: str = "../../llm_results") -> Dict:
    """
...
def evaluate(mention_map: Dict[str, Dict[str, str]],
             prediction_pairs: List[Tuple[str, str]],
             similarity_matrix: np.ndarray,
             tmp_folder: str = '../../tmp/') -> Dict[str, Tuple[float, float, float]]:
    """
    ...
    curr_gold_cluster_map = [(men, mention_map[men]['gold_cluster']) for men in curr_mentions]
